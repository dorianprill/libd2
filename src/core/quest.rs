...
pub fn parse_legacy_quest_words(raw: &[u8]) -> Option<[[u16; SAVE_QUEST_WORDS_PER_DIFFICULTY]; 3]> {
    let marker = raw
...
mod tests {
    ...
    fn progression_tracks_completed_difficulties() {
        ...
        super::apply_progression_from_quests(&mut raw, &quests);
    ...
    fn legacy_quest_words_parse_and_write_round_trip() {
        ...
        assert!(write_legacy_quest_words(&mut raw, &quests));
        assert_eq!(parse_legacy_quest_words(&raw), Some(quests));
    }
